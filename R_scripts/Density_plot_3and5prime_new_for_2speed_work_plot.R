...
# add quadrants
plot = plot + geom_vline(data = FIRS, aes(xintercept = median_low), linetype="dashed", size=1, color = "dodgerblue4")
plot = plot + geom_hline(data = FIRS, aes(yintercept = median_up), linetype="dashed", size=1, color = "dodgerblue4")
