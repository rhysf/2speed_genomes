...
option_list = list(
    ...
    make_option(c("-g", "--highlight"), default=NULL, help="Highlight this GO term"),
...
opt_parser = OptionParser(usage = "Usage: %prog -d <dataframe> -g all
Optional: -o Output [opt$dataframe-densityplot_recolored.pdf]
          -i Height of pdf [8]
          -w Width of pdf [12]

Notes: Dataframe should look like this:
scaffold0001    BSAL2_000002      GO:0001,GO:0002      -       1271    221     3.10414555055401        2.34439227368511
scaffold0001    BSAL2_000003      GO:0001      +       1069    221     3.02897770520878        2.34439227368511", option_list=option_list);
...
table_highlighted <- table %>%
  mutate(highlight = str_detect(desc, opt$highlight))

# colours
red_palette <- c("darkred", "firebrick", "indianred")
blue_palette <- c("steelblue", "cornflowerblue", "royalblue")
green_palette <- c("darkgreen", "forestgreen", "limegreen")
purple_palette <- c("darkorchid", "mediumorchid", "lavenderblush")

# Combine the three palettes for use in scale_fill_gradient2
combined_palette <- c(red_palette, blue_palette, green_palette, purple_palette)


#plot with different bins
plot=ggplot(table_highlighted, aes(x=downlog10, y=uplog10)) +
...
# add quadrants
plot <- plot +
  geom_vline(data = FIRS, aes(xintercept = median_low), linetype = "dashed", linewidth = 1, color = "dodgerblue4") +
  geom_hline(data = FIRS, aes(yintercept = median_up), linetype = "dashed", linewidth = 1, color = "dodgerblue4")
